        ...
        while (sc.hasNext()){
            ...
            for (Integer r : listR) {
                ...
                for (int i = 0; i < listI.size(); i++) {
                    if (listI.get(i).contains(r.toString())){
                ...
                if (count != 0){
                    num += map.size() * 2 + 2;
                    sb.append(r.toString() +" " +count + " ");
                }

                Set<Integer> integers = map.keySet();
                for (Integer integer : integers) {
                    String str = map.get(integer);
                    sb.append(integer +" " + str + " ");
                }
            }
     ...
     * 6 cab ad abcd cba abc bca abc 1 输出：3 bca
     ...
     public static void main(String arg[]) {
        ...
        while (sc.hasNext()) {
            ...
            String source = arr[arr.length - 2];
           ...
           List<String> list = new ArrayList<>();
            ...
            int num = 0;
            for (String st : list) {
                if (st.length() != source.length() || st.equals(source)){
                    continue;
                }

                char[] tarArr = st.toCharArray();
                Arrays.sort(sourArr);
                Arrays.sort(tarArr);
                if (!Arrays.equals(sourArr,tarArr)){
                    continue;
                }
                num += 1;
                if (num == index){
...
#### HJ30 字符串合并处理
    ...
    3.排序后的字符串中‘0’-‘9’；‘a’-‘f’; ‘A’-‘F’转换，对以上16进制的字符转为二进制并倒叙，再转为16进制，再大写输出；
      输入： ‘dec’ “fab” 输出：“5D37BF”  输入：“ab CD” 输出：“3B5D”；
            Eqr v9oEb12U2ur4xu7rd931G1f50qDo   8084842CAE9B9G7D7BUFooqqrrrvuxu
